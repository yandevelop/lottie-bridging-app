...
@objc(LottieAnimationView) class objc_wrapper_AnimationView: UIView {
    ...
    @objc func loadAnimation(path animationPath: String) {
        let animation = LottieAnimation.filepath(animationPath, animationCache: DefaultAnimationCache.sharedCache)
    ...
    @objc func setLoopMode(_ loopMode: LottieLoopMode, count: CGFloat) {
        switch loopMode {
            case .playOnce: break
            case .loop: break
    ...
    @objc func logHierarchyKeypaths() {
        NSLog("Hi! %@", animationView.allHierarchyKeypaths())
    }
